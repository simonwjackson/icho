...
local function fetch_rate_limits_async(config, callback)
  if fetch_in_progress then return end
  ...
  local cmd = {
    ...
    "-H", string.format("anthropic-anonymous-id: %s", config.anonymous_id),
    "-H", "anthropic-client-platform: web_claude_ai",
    "-H", "anthropic-client-version: 1.0.0",
    "-H", string.format("anthropic-device-id: %s", config.device_id),
    "-H", "content-type: application/json",
    "-H", "Alt-Used: claude.ai",
    "-H", "Connection: keep-alive",
    "-H", string.format("Cookie: sessionKey=%s; anthropic-device-id=%s; ajs_anonymous_id=%s",
      config.session_key, config.device_id, config.anonymous_id),
...
function M.calculate_pace(utilization, resets_at)
  local now = os.time() * 1000
  local reset_time = M.parse_iso_date(resets_at)
  if not reset_time then return 0 end
