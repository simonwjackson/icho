...
local function has_claude_usage(self)
	...
	if not data or not data.seven_day then
		return false
	end
...
M.ClaudeWeekly = {
	...
	hl = function(self)
		local severity = claude.get_weekly_severity(self.seven_day.utilization, self.seven_day.resets_at)
...
M.ClaudePace = {
	condition = has_claude_usage,
	provider = function(self)
		local pace = claude.calculate_pace(self.seven_day.utilization, self.seven_day.resets_at)
...
M.ClaudeBudget = {
	condition = has_claude_usage,
	provider = function(self)
		local budget = claude.calculate_daily_budget(self.seven_day.utilization, self.seven_day.resets_at)
